...
- .filter() : returns a new array that includes the items that return true at the callback function.<br/>More at functions  
...
- **filter()** : is a  method that creates an array filled with all array elements that pass a test (provided as a function).<br/>
  ...
  *Filter for object* :
  ...
  ```
- **reduce()**: is a method that reduces the array to a single value. <br/>
  The single value that is returned can be of any type, means  reduce() can be used to transform an input array into any output you desire. <br/>
  ...
  Note: this method does not change the original array.<br/>
  Note: If you don’t pass in an initial value, reduce will assume the first item in your array is your initial value
  ...
  *InitialValue Omitted* :<br/>
  If initialValue is not provided, the iteration will start at the second element in the array (at index 1)    
  ...
  */
  ```
  *Returning and Array* :<br/> Generally, if you plan to return an array, map() is often a better option. 
  ```js
